...
from ansible_collections.netapp.storagegrid.plugins.modules.na_sg_grid_ilm_pool import (
    ILM_pool as grid_ilm_pool_module,
)
...
SRR = {
    ...
    "api_response_creation_succeeded": (
        {
            "data": {
                "displayName": "pool1",
                "disks": [{"group": 1, "grade": 1, "siteId": "00000000-0000-0000-0000-000000000000"}],
                "archives": [{"group": 1, "grade": 1, "siteId": "00000000-0000-0000-0000-000000000000"}],
                "id": "p1",
            }
        },
        None,
    ),
    "api_response_existing_ilm_pool": (
        {
            "data": [
                {
                    "displayName": "pool1",
    ...
    "api_response_existing_ilm_pool_updated": (
        {
            "data": {
                "displayName": "pool1",
                "disks": [
                    {"group": 2, "grade": 2, "siteId": "00000000-0000-0000-0000-000000000002"},
                ],
                "archives": [{"group": 1, "grade": 1, "siteId": "00000000-0000-0000-0000-000000000000"}],
                "id": "p1",
            }
...
class TestMyModule(unittest.TestCase):
    ...
    def set_default_args_fail_check(self):
        return dict({"name": "pool1", "auth_token": "01234567-5678-9abc-78de-9fgabc123def", "validate_certs": False})  # missing "api_url"
    ...
    def set_args_create_na_sg_grid_ilm_pool(self):
        return dict(
            {
                "state": "present",
                "name": "pool1",
    ...
    def set_args_delete_na_sg_grid_ilm_pool(self):
        return dict(
            {
                "state": "absent",
                "name": "pool1",
    ...
    @patch("ansible_collections.netapp.storagegrid.plugins.module_utils.netapp.SGRestAPI.send_request")
    def test_create_na_sg_grid_ilm_pool_pass(self, mock_request):
        set_module_args(self.set_args_create_na_sg_grid_ilm_pool())
        mock_request.side_effect = [
            SRR["version_114"],
            SRR["empty_good"],  # get
    ...
    @patch("ansible_collections.netapp.storagegrid.plugins.module_utils.netapp.SGRestAPI.send_request")
    def test_idempotent_create_na_sg_grid_ilm_pool_pass(self, mock_request):
        set_module_args(self.set_args_create_na_sg_grid_ilm_pool())
        mock_request.side_effect = [
            SRR["version_114"],
            SRR["api_response_existing_ilm_pool"],  # get
            SRR["end_of_sequence"],
        ]
        my_obj = grid_ilm_pool_module()
        with pytest.raises(AnsibleExitJson) as exc:
            my_obj.apply()
        print("Info: test_idempotent_create_na_sg_grid_ilm_pool_pass: %s" % repr(exc.value.args[0]))
        assert not exc.value.args[0]["changed"]

    @patch("ansible_collections.netapp.storagegrid.plugins.module_utils.netapp.SGRestAPI.send_request")
    def test_update_na_sg_grid_ilm_pool_pass(self, mock_request):
        args = self.set_args_create_na_sg_grid_ilm_pool()
        args.update(
            {
                "disks": [
                    {"group": 2, "grade": 2, "siteId": "00000000-0000-0000-0000-000000000002"},
        ...
        mock_request.side_effect = [
            SRR["version_114"],
            SRR["api_response_existing_ilm_pool"],  # get
            SRR["api_response_existing_ilm_pool_updated"],  # put
            SRR["end_of_sequence"],
        ]
        my_obj = grid_ilm_pool_module()
        with pytest.raises(AnsibleExitJson) as exc:
    ...
    @patch("ansible_collections.netapp.storagegrid.plugins.module_utils.netapp.SGRestAPI.send_request")
    def test_delete_na_sg_grid_ilm_pool_pass(self, mock_request):
        set_module_args(self.set_args_delete_na_sg_grid_ilm_pool())
        mock_request.side_effect = [
            SRR["version_114"],
            SRR["api_response_existing_ilm_pool"],  # get
        ...
        with pytest.raises(AnsibleExitJson) as exc:
            my_obj.apply()
        print("Info: test_delete_na_sg_grid_ilm_pool_pass: %s" % repr(exc.value.args[0]))
        assert exc.value.args[0]["changed"]
