...
""" unit tests NetApp StorageGRID Autosupport Ansible module: na_sg_grid_proxy_settings"""
...
from ansible_collections.netapp.storagegrid.plugins.modules.na_sg_grid_proxy_settings import (
    SgGridProxySetting as grid_proxy_settings_module,
)
...
SRR = {
    ...
    "api_response_existing_proxy_updated": (
        {
            "data": {
                "enable": True,
                "hostname": "proxy.example.com",
                "hostPort": 8081,
                "proxy": "http://myproxy1.example.com:8080"
            }
        },
        None,
    ),
...
class TestMyModule(unittest.TestCase):
    ...
    def set_default_args_fail_check(self):
        return dict({"enable": True, "hostname": "proxy.example.com", "hostPort": 8081, "proxy": "http://myproxy.example.com:8080", })
    ...
    def set_args_update_na_sg_grid_proxy_pass_check(self):
        return dict(
            {
                "state": "present",
                "enable": True,
                "host_name": "proxy.example.com",
                "host_port": 8080,
                "proxy": "http://myproxy1.example.com:8081",
                "api_url": "gmi.example.com",
                "auth_token": "01234567-5678-9abc-78de-9fgabc123def",
                "validate_certs": False,
            }
    ...
    @patch("ansible_collections.netapp.storagegrid.plugins.module_utils.netapp.SGRestAPI.send_request")
    def test_update_na_sg_grid_proxy_setting_pass(self, mock_request):
        args = self.set_args_update_na_sg_grid_proxy_pass_check()
        # args["enable"] = True
        args["host_port"] = 8081
        args["proxy"] = "http://myproxy1.example.com:8080"
        ...
        my_obj = grid_proxy_settings_module()
        with pytest.raises(AnsibleExitJson) as exc:
            my_obj.apply()
        print("Info: test_update_na_sg_grid_proxy_setting_pass: %s" % repr(exc.value.args[0]))
        assert exc.value.args[0]["changed"]
    ...
    def test_idempotent_update_na_sg_grid_proxy_setting_pass(self, mock_request):
        ...
        with pytest.raises(AnsibleExitJson) as exc:
            my_obj.apply()
        print("Info: test_idempotent_update_na_sg_grid_proxy_setting_pass: %s" % repr(exc.value.args[0]))
