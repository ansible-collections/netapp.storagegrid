# (c) 2026, NetApp, Inc
# GNU General Public License v3.0+ (see COPYING or https://www.gnu.org/licenses/gpl-3.0.txt)

""" unit tests NetApp StorageGRID Single Sign-On Ansible module: na_sg_grid_sso"""

from __future__ import absolute_import, division, print_function

__metaclass__ = type
import json
import pytest
import sys

from ansible_collections.netapp.storagegrid.tests.unit.compat import unittest
from ansible_collections.netapp.storagegrid.tests.unit.compat.mock import patch
from ansible.module_utils import basic
from ansible.module_utils._text import to_bytes
from ansible_collections.netapp.storagegrid.plugins.modules.na_sg_grid_sso import (
    SgSSO as grid_sso_module,
)

if sys.version_info < (3, 11):
    pytestmark = pytest.mark.skip("Skipping Unit Tests on 3.11")

# REST API canned responses when mocking send_request
SRR = {
    # common responses
    "empty_good": ({"data": []}, None),
    "not_found": (
        {"status": "error", "code": 404, "data": {}},
        {"key": "error.404"},
    ),
    "version_114": ({"data": {"productVersion": "11.4.0-20200721.1338.d3969b3"}}, None),
    "end_of_sequence": (None, "Unexpected call to send_request"),
    "generic_error": (None, "Expected error"),
    "delete_good": ({"code": 204}, None),
    "node_health": (
        {
            "data": [
                {
                    "id": "0b1866ed-d6e7-41b4-815f-bf867348b76b",
                    "name": "dc1-vm-adm",
                    "type": "adminNode",
                },
                {
                    "id": "1c2877fe-e7f8-52c5-926f-cf978459c87c",
                    "name": "dc1-vm-storage",
                    "type": "storageNode",
                },
            ]
        },
        None,
    ),
    "grid_get_empty_sso": ({"data": {}}, None),
    "grid_sso_configured": (
        {
            "data": {
                "disable": False,
                "sandbox": True,
                "identityProvider": {
                    "type": "Azure",
                    "federationServiceName": "www.example.com",
                    "relyingPartyIdentifier": "MyStorageGRID-[HOSTNAME]",
                    "adminNodes": [
                        {
                            "nodeId": "25e2700e-ccd9-4274-91c4-daf11bef21b8",
                            "federationMetadataUrl": "https://login.microsoftonline.com/0000ml?appid=00000000-0000-0000-0000-000000000000"
                        }
                    ]
                },
                "disableTLS": False,
                "caCert": None
            }
        },
        None,
    ),
    "grid_sso_updated": (
        {
            "data": {
                "disable": False,
                "sandbox": False,
                "disableTLS": True,
                "caCert": "",
                "identityProvider": {
                    "type": "adfs",
                    "federationServiceName": "ansible_test_federation",
                    "relyingPartyIdentifier": "ansible_test_relying_party",
                    "adminNodes": [
                        {
                            "nodeId": "0b1866ed-d6e7-41b4-815f-bf867348b76b",
                            "federationMetadataUrl": "https://ansible_test_metadata_url",
                        }
                    ],
                },
            }
        },
        None,
    ),
}


def set_module_args(args):
    """prepare arguments so that they will be picked up during module creation"""
    args = json.dumps({"ANSIBLE_MODULE_ARGS": args})
    basic._ANSIBLE_ARGS = to_bytes(args)  # pylint: disable=protected-access


class AnsibleExitJson(Exception):
    """Exception class to be raised by module.exit_json and caught by the test case"""

    pass


class AnsibleFailJson(Exception):
    """Exception class to be raised by module.fail_json and caught by the test case"""

    pass


def exit_json(*args, **kwargs):  # pylint: disable=unused-argument
    """function to patch over exit_json; package return data into an exception"""
    if "changed" not in kwargs:
        kwargs["changed"] = False
    raise AnsibleExitJson(kwargs)


def fail_json(*args, **kwargs):  # pylint: disable=unused-argument
    """function to patch over fail_json; package return data into an exception"""
    kwargs["failed"] = True
    raise AnsibleFailJson(kwargs)


class TestMyModule(unittest.TestCase):
    """a group of related Unit Tests"""

    def setUp(self):
        self.mock_module_helper = patch.multiple(basic.AnsibleModule, exit_json=exit_json, fail_json=fail_json)
        self.mock_module_helper.start()
        self.addCleanup(self.mock_module_helper.stop)

    def set_default_args_fail_check(self):
        return dict(
            {
                "disable": False,
                "sandbox": True,
                "auth_token": "01234567-5678-9abc-78de-9fgabc123def",
                "validate_certs": False,
            }
        )

    def set_default_args_pass_check(self):
        return dict(
            {
                "state": "present",
                "disable": False,
                "sandbox": True,
                "disable_tls": True,
                "identity_provider": {
                    "idp_type": "adfs",
                    "federation_service_name": "ansible_test_federation",
                    "relying_party_identifier": "ansible_test_relying_party",
                    "admin_nodes": [
                        {
                            "name": "dc1-vm-adm",
                            "federation_metadata_url": "https://ansible_test_metadata_url",
                        }
                    ],
                },
                "api_url": "gmi.example.com",
                "auth_token": "01234567-5678-9abc-78de-9fgabc123def",
                "validate_certs": False,
            }
        )

    def set_args_set_na_sg_grid_sso(self):
        return dict(
            {
                "state": "present",
                "disable": False,
                "sandbox": True,
                "disable_tls": True,
                "identity_provider": {
                    "idp_type": "adfs",
                    "federation_service_name": "ansible_test_federation",
                    "relying_party_identifier": "ansible_test_relying_party",
                    "admin_nodes": [
                        {
                            "name": "dc1-vm-adm",
                            "federation_metadata_url": "https://ansible_test_metadata_url",
                        }
                    ],
                },
                "api_url": "gmi.example.com",
                "auth_token": "01234567-5678-9abc-78de-9fgabc123def",
                "validate_certs": False,
            }
        )

    def set_args_update_na_sg_grid_sso(self):
        return dict(
            {
                "state": "present",
                "disable": True,
                "sandbox": True,
                "disable_tls": True,
                "identity_provider": {
                    "idp_type": "adfs",
                    "federation_service_name": "ansible_test_federation",
                    "relying_party_identifier": "ansible_test_relying_party",
                    "admin_nodes": [
                        {
                            "name": "dc1-vm-adm",
                            "federation_metadata_url": "https://ansible_test_metadata_url",
                        }
                    ],
                },
                "api_url": "gmi.example.com",
                "auth_token": "01234567-5678-9abc-78de-9fgabc123def",
                "validate_certs": False,
            }
        )

    def test_module_fail_when_required_args_missing(self):
        """required arguments are reported as errors"""
        with pytest.raises(AnsibleFailJson) as exc:
            set_module_args(self.set_default_args_fail_check())
            grid_sso_module()
        print("Info: test_module_fail_when_required_args_missing: %s" % exc.value.args[0]["msg"])

    @patch("ansible_collections.netapp.storagegrid.plugins.module_utils.netapp.SGRestAPI.send_request")
    def test_module_fail_when_required_args_present(self, mock_request):
        """required arguments are reported as errors"""
        mock_request.side_effect = [
            SRR["version_114"],
            SRR["node_health"],
        ]
        with pytest.raises(AnsibleExitJson) as exc:
            set_module_args(self.set_default_args_pass_check())
            grid_sso_module()
            exit_json(changed=True, msg="Induced arguments check")
        print("Info: test_module_fail_when_required_args_present: %s" % exc.value.args[0]["msg"])
        assert exc.value.args[0]["changed"]

    @patch("ansible_collections.netapp.storagegrid.plugins.module_utils.netapp.SGRestAPI.send_request")
    def test_set_na_sg_grid_sso_pass(self, mock_request):
        set_module_args(self.set_args_set_na_sg_grid_sso())
        mock_request.side_effect = [
            SRR["version_114"],
            SRR["node_health"],
            SRR["grid_sso_configured"],
            SRR["grid_sso_configured"],
            SRR["end_of_sequence"],
        ]
        my_obj = grid_sso_module()
        with pytest.raises(AnsibleExitJson) as exc:
            my_obj.apply()
        print("Info: test_set_na_sg_grid_sso_pass: %s" % repr(exc.value.args[0]))
        assert exc.value.args[0]["changed"]

    @patch("ansible_collections.netapp.storagegrid.plugins.module_utils.netapp.SGRestAPI.send_request")
    def test_update_na_sg_grid_sso_pass(self, mock_request):
        set_module_args(self.set_args_update_na_sg_grid_sso())
        mock_request.side_effect = [
            SRR["version_114"],
            SRR["node_health"],  # get node-health (build_admin_nodes)
            SRR["grid_sso_configured"],  # get current sso config
            SRR["grid_sso_updated"],  # put updated sso config
            SRR["end_of_sequence"],
        ]
        my_obj = grid_sso_module()
        with pytest.raises(AnsibleExitJson) as exc:
            my_obj.apply()
        print("Info: test_disable_na_sg_grid_sso_pass: %s" % repr(exc.value.args[0]))
        assert exc.value.args[0]["changed"]
